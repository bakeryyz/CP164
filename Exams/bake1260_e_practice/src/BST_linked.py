...
class BST:
    ...
    def _node_counts_aux(self, node):
        ...
        if node is None:
            ...
            l_zero, l_one, l_two = self._node_counts_aux(node._left)
            r_zero, r_one, r_two =self._node_counts_aux(node._right)
            
            if node._left is None and node._right is None:
                # Node has no children
                zero = l_zero + r_zero + 1
                one = l_one + r_one
                two = l_two + r_two
    ...
    def is_valid(self):
        ...
        # your code here
        valid = self._is_valid_aux(self._root, float('-inf'), float('inf'))
        return valid
    ...
    def _is_valid_aux(self, node, min_value, max_value):
        ...
        valid = True
        if node is not None:
            # check if value is greater than left child and less than right child
            if not (min_value < node._value < max_value):
                valid = False

            # check if height is correct
            if valid:
                # expected height depends on the height of the left and right children
            ...
            # start with left child/tree
            if valid:
                valid = self._is_valid_aux(node._left, min_value, node._value)

            # start with right child/tree (only check if left child/tree is valid)
            if valid:
                valid = self._is_valid_aux(node._right, node._value, max_value)
