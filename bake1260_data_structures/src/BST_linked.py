...
class BST:
    ...
    def __contains__(self, key):
        ...
        while curr is not None and check:
            if curr._value == key:
                check = False
    ...
    def _eq_aux(self, check1, check2):
        """
        ---------------------------------------------------------
        Auxillary function for the Leaf_Count function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
            SEWY
        ---------------------------------------------------------
        """
        ...
        if check1 is None and check2 is None:
            equals = True
        elif check1 is not None and check2 is not None:
            if check1._value == check2._value:
                equals = self._eq_aux(check1._left, check2._left)
                if equals:
                    equals = self._eq_aux(check1._right, check2._right)
        return equals
    ...
    def _parent_r_aux(self, check, key, check2):
        """
        ---------------------------------------------------------
        Auxillary function for the Parent function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        if check is None:
            value = None
        elif key < check._value:
            value = self._parent_r_aux(check._left, key, check)
        elif key > check._value:
            value = self._parent_r_aux(check._right, key, check)
        elif check2 is None:
            value = None
        else:
            value = deepcopy(check2._value)
        return value
    ...
    def max(self):
        ...
        check = self._root
        while check._right is not None:
            check = check._right
    ...
    def leaf_count(self):
        ...
        if self._root is None:
            ...
            check = self._root
            count = self._leaf_count_aux(check)
        return count

    def _leaf_count_aux(self, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Leaf_Count function.
        ---------------------------------------------------------
        ...
        if check is None:
            count = 0
        elif check._left is None and check._right is None:
            count = 1
        else:
            count = self._leaf_count_aux(
                check._left) + self._leaf_count_aux(check._right)
        return count
    ...
    def _two_child_count_aux(self, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Two_Child_Count function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        if check is None:
            count = 0
        elif check._left is not None and check._right is not None:
            count = 1 + \
                self._two_child_count_aux(
                    check._left) + self._two_child_count_aux(check._right)
        else:
    ...
    def _one_child_count_aux(self, check):
        """
        ---------------------------------------------------------
        Auxillary function for the One_Child_Count function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        if check is None:
            count = 0
        elif check._left is None and check._right is not None:
            count = 1 + self._one_child_count_aux(check._right)
        elif check._left is not None and check._right is None:
            count = 1 + self._one_child_count_aux(check._left)
        else:
            count = self._one_child_count_aux(
                check._left) + self._one_child_count_aux(check._right)
        return count
    ...
    def _node_counts_aux(self, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Node_Counts function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        if check is None:
            ...
            zero, one, two = self._node_counts_aux(check._left)
            zero += 1
        elif check._right is not None and check._left is None:
            zero, one, two = self._node_counts_aux(check._right)
            zero += 1
        else:
            zero, one, two = self._node_counts_aux(check._left)
            uno, dos, tres = self._node_counts_aux(check._right)
            zero += uno
            one += dos
            two += tres
        return zero, one, two
    ...
    def _is_balanced_aux(self, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Is_Balanced function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        if check is None:
            balanced = True
        elif abs(self._node_height(check._left) - self._node_height(check._right)) > 1:
            balanced = False
        else:
            balanced = self._is_balanced_aux(
                check._left) and self._is_balanced_aux(check._right)
        return balanced
    ...
    def _is_valid_aux(self, check, check2, check3):
        """
        ---------------------------------------------------------
        Auxillary function for the Is_Valid function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        if check is None:
            valid = True
        elif check2 is not None and check._value <= check2._value:
            valid = False
        elif check3 is not None and check._value >= check3._value:
            valid = False
        elif check._height != max(self._node_height(check._left),
                                  self._node_height(check._right)) + 1:
            valid = False
        else:
            valid = self._is_valid_aux(check._left, check2, check) and self._is_valid_aux(
                check._right, check, check3)
        return valid
    ...
    def _inorder_aux(self, root, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Inorder function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        """
        if root is not None:
            self._inorder_aux(root._left, check)
            check.append(deepcopy(root._value))
            self._inorder_aux(root._right, check)
        return
    ...
    def _preorder_aux(self, root, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Preorder function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        """
        if root is not None:
            check.append(root._value)
            self._preorder_aux(root._left, check)
            self._preorder_aux(root._right, check)
        return
    ...
    def _postorder_aux(self, root, check):
        """
        ---------------------------------------------------------
        Auxillary function for the Postorder function.
        ---------------------------------------------------------
        Parameters:
            SIUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU
        Returns:
        ...
        """
        if root is not None:
            self._postorder_aux(root._left, check)
            self._postorder_aux(root._right, check)
            check.append(root._value)
        return
    ...
    def levelorder(self):
        ...
        if self._root is not None:
            check = []
            check.append(self._root)

            while len(check) > 0:
                value = check.pop(0)
                values.append(value._value)

                if value._left is not None:
                    check.append(value._left)
                if value._right is not None:
                    check.append(value._right)
        return values
