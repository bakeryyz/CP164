...
class List:
    ...
    def remove_many(self, key):
        ...
        -------------------------------------------------------
        """
        # handle for matches at the front.
        ...
        if self._front is not None:
            ...
            while current._next is not None:
                if current._next._value == key:
                    ...
                    if current._next is None: # last node
                        self._rear = current
                else:
        ...
        if self._front is not None:
            ...
            while current is not None:
                ...
                while following is not None:
                    if following._value == value:
                        # remove duplicate
                        previous._next = following._next
                        if following == self._rear:
                            self._rear = previous
                        self._count -= 1
                        following = previous._next
                    else:
                        previous = following
    ...
    def split(self):
        ...
        mid_point = self._count // 2 + self._count % 2
    ...
    def intersection(self, source1, source2):
        ...
        -------------------------------------------------------
        """
        # start with first node from source1
        source1_node = source1._front
        while source1_node is not None:
            value = source1_node._value

            # check if value is in source2
            source2_node = source2._front
            found = False
            while source2_node is not None and not found:
                if source2_node._value == value:
                    found = True
                else:
                    source2_node = source2_node._next

            # if found, then we have the intersection value 
            # which we have to check if present in self and append to it only if it already isn't present.
            if found:
                self_node = self._front
                already_present = False
                while self_node is not None and not already_present:
                    if self_node._value == value:
    ...
    def union(self, source1, source2):
        ...
        -------------------------------------------------------
        """
        # Add values from source1
        source1_node = source1._front
        while source1_node is not None:
            value = source1_node._value

            # check if value is in self
            self_node = self._front
            found = False
            while self_node is not None and not found:
                if self_node._value == value:
                    found = True
                else:
                    self_node = self_node._next
