...
def read_movie(line):
    ...
    title, year, director, rating, genre_codes = line.split("|")
    year = int(year)
    genres = []
    for i in genre_codes.split(","):
        genres.append(int(i))
    rating = float(rating)
    movie = Movie(title, year, director, rating, genres)
...
def genre_counts(movies):
    ...
    for genre in Movie.GENRE_CODES:
        counter = 0
        for movie in movies:
            if genre in movie.genres:
                counter += 1
